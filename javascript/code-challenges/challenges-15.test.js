...
const toTitleCase = (arr) => {
  ...
  arr.map(function (v) {
    let value = v.charAt(0).toUpperCase() + v.slice(1);
...
const isSecure = (url) => {
  // Solution code here...
  let happy = url.startsWith('https://', 0);
  return happy;
};
...
const detectTicTacToeWin = (board) => {
  ...
  let topLeft = checker(board[0][0], board[1][1], board[2][2]);
  let topRight = checker(board[0][2], board[1][1], board[2][0]);

  if (topLeft || topRight) {
    return true;
  }

  board.map(function (v, i) {

    let horizontal = checker(v[0], v[1], v[2]);
    let vertical = checker(board[0][i], board[1][i], board[2][i]);
