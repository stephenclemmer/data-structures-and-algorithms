...
function transformToLis(obj) {
  ...
  keys.forEach((key, index) => {
    arr.push(`<li>${key}: ${values[index]}</li>`);
  });
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let result = input.map(function (key) {


    let filtered = key.filter(n => typeof n === 'number')

    let notFive = filtered.filter(v => ((Number(v) % 5) === 0))

    let answer = notFive.map((v, i) => {
